...
public final class OperationExecutorProducer implements LifecycleExecutorProducer {
  ...
  public OperationExecutorProducer(
      Repository repository, TasksInfoStorage storage, ProcessorFactory factory) {
  ...
  public LifecycleExecutor getExecutor(int amount, int threads) {
    return new OperationExecutor.OperationExecutorBuilder(
            new CountDownLatch(storage.get().size()), threads, amount, repository, storage, factory)
        .build();
  }
}
